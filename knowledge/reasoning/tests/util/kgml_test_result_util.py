#!/usr/bin/env python3
"""
Utility functions for the KGML Test Results Viewer
"""
...
logger = logging.getLogger("KGMLViewerUtils")
...
def format_runs_for_table(runs: List[Dict[str, Any]]) -> List[List]:
    """Format runs data for display in a table"""
    if not runs:
        return []
    return [
        [
            run.get("formatted_date", ""),
            run.get("model_name", "unknown"),
            run.get("success_rate", ""),
            run.get("path", "")  # Keep path for reference but will be hidden in UI
        ]
        for run in runs
    ]
...
def format_tests_for_table(run_data: Dict[str, Any]) -> List[List]:
    """Format tests data for display in a table"""
    if "error" in run_data or not run_data.get("tests"):
        return []
    ...
    return [
        [
            test.get("name", ""),
            test.get("status", ""),
            test.get("valid_total", ""),
            test.get("path", "")  # Keep path for reference but will be hidden in UI
        ]
...
def format_iterations_for_table(iterations: List[Dict[str, Any]]) -> List[List]:
    """Format iterations data for display in a table"""
    if not iterations:
        return []
    ...
    return [
        [
            ...
            iteration.get("response_time", ""),
            iteration.get("path", "")  # Keep path for reference but will be hidden in UI
        ]
