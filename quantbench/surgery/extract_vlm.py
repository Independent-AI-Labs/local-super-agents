...
from transformers import (
    ...
    Qwen2VLProcessor,
    AutoProcessor,
    Qwen2VLConfig, AutoConfig, AutoModel
)
...
def main(args):
    ...
    # Check if a model input directory is specified
    if MODEL_INPUT_DIR is not None:
        model_path = MODEL_INPUT_DIR
        local_model = True
        print(f"Loading model from local directory: {model_path}")
        qwen2vl = Qwen2VLForConditionalGeneration.from_pretrained(
            model_path, torch_dtype=dtype, ignore_mismatched_sizes=True
        )
        # auto_config = AutoConfig.from_pretrained(MODEL_INPUT_DIR)
        ...
        # qwen2vl = AutoModel.from_pretrained(MODEL_INPUT_DIR, config=auto_config)
    else:
...
if __name__ == "__main__":
    ...
    # Set the global MODEL_INPUT_DIR if provided
    if args.input_dir:
