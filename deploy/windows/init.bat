...
if exist "7z2409-x64.exe" (
    ...
    echo 7z2409-x64.exe not found in %install_path%.
    pause
    exit /b
)
echo.
...
:: Extract mvp.7z into 'agents' directory
echo Extracting mvp.7z into 'agents' directory...
if exist "mvp.7z" (
    "%ProgramFiles%\7-Zip\7z.exe" x "mvp.7z" -o"%install_path%\agents" -y
    if %errorlevel% neq 0 (
        echo Failed to extract mvp.7z.
        pause
        exit /b
    )
    echo Extraction completed.
) else (
    echo mvp.7z not found in %install_path%.
    pause
    exit /b
)
echo.
...
if exist "%install_path%\agents\mvp\res\run\web.7z" (
    "%ProgramFiles%\7-Zip\7z.exe" x "%install_path%\agents\mvp\res\run\web.7z" -o"%install_path%\tools" -y
    if %errorlevel% neq 0 (
...
if exist "%install_path%\agents\mvp\res\vm\HyperV.7z.001" (
    "%ProgramFiles%\7-Zip\7z.exe" x "%install_path%\agents\mvp\res\vm\HyperV.7z.001" -o"%install_path%\agents\mvp\res\vm" -y
    if %errorlevel% neq 0 (
...
if exist "%install_path%\agents\mvp\res\vm\vm_import.ps1" (
    powershell -ExecutionPolicy Bypass -File "%install_path%\agents\mvp\res\vm\vm_import.ps1"
    if %errorlevel% neq 0 (
...
rmdir /s /q "%install_path%\agents\mvp\res\vm\HyperV"
...
if exist "%install_path%\agents\mvp\res\pre\pre_install.bat" (
    call "%install_path%\agents\mvp\res\pre\pre_install.bat"
    if %errorlevel% neq 0 (
...
if exist "%install_path%\agents\mvp\res\envs\init_python_env.bat" (
    call "%install_path%\agents\mvp\res\envs\init_python_env.bat"
    if %errorlevel% neq 0 (
...
if exist "%install_path%\agents\mvp\requirements.txt" (
